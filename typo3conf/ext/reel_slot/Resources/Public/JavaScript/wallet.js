/**
 * Reel Slot – die Verrechnung am Gerät
 * ====================================
 *
 ...
 *
 * destroy() hängt am pagehide-Weg: reel-slot.js meldet teardown() für pagehide
 ...
 * ------------------------------------------
 *   data-rs-machine-credit   der Gerätekredit
 *   data-rs-mirror           der rohe Spiegelwert aus dem Browserspeicher
 *
 * Und KEINE anderen. data-rs-bank, data-rs-total und data-rs-cashout gehören
 ...
 * DOM-Ereignisse an"):
 *
 *   rs:round   abbrechbar, vor dem Anlaufen → hier wird geprüft und abgebucht
 *   rs:result  nach der Auswertung          → hier wird gutgeschrieben
 *   rs:state   jeder Zustandswechsel        → hier wird die Einsatzwahl gesperrt
 *   rs:collect eigenes Ereignis aus payout.js → Kappung melden
 *
 ...
 * ====================================================================
 *
 * Das Problem in einem Satz: rs:round ist abbrechbar und muss deshalb SOFORT
 ...
 *   Der Gewinn              wird NICHT ausgezahlt. rs:payout wird gar nicht
 ...
 *                           der nächste Hebelzug läuft durch das normale Veto.
 ...
 *    Abschnitt 3.3 schon beim Hebelzug fest.
 *  · Vor dem Anlaufen auf die Bestätigung warten? Das ist die saubere Lösung
 *    für ein echtes Konto, kostet aber eine Wartezeit zwischen Hebelzug und
 ...
 * ABBRECHBARES rs:payout mit einem WinClaim im detail. Sagt niemand etwas,
...
import { NixieCounter } from '@phomo17/reel-slot/counter.js';
import { MessageBoard } from '@phomo17/reel-slot/message.js';
import { WinClaim } from '@phomo17/reel-slot/payout.js';
import { CoinSlot } from '@phomo17/reel-slot/coinslot.js';

/**
 * Der Schlüssel, unter dem der Gerätekredit dieses Automaten geführt wird.
 *
 * Wortgleich zu ReelSlot::IDENTIFIER in Classes/ReelSlot.php. Er kommt vom
 ...
 * casinoKunterbunt.machine.reel_slot (B.5.3).
 */
const MACHINE_KEY = 'reel_slot';
 ...
 *
 * Phase 8 hat entgegen der früheren Vermutung KEINEN Zustand „risk"
 ...
 * vorbei. Für Phase 9 gilt die Aussage oben unverändert weiter.
 */
...
export class Wallet {
	...
	constructor(root) {
		...
		this.board = new MessageBoard(cabinet.querySelector('.rs-message'));
		...
		this.betsGroup = cabinet.querySelector('.rs-bets');
		/** @type {HTMLElement[]} */
		this.betButtons = [...cabinet.querySelectorAll('.rs-bet[data-rs-bet]')];
		...
		/**
		 * Das Versprechen der laufenden Abbuchung. Wird in rs:result abgewartet,
		...
		this.listen(root, 'rs:round', (event) => this.onRound(event));
		this.listen(root, 'rs:result', (event) => { void this.onResult(event); });
	...
	selectBet(button) {
		if (this.locked) {
			...
			// sichtbar (.rs-bets--locked), aber verlassen wird sich auf sie
			// nicht: eine Sperre, die nur aus CSS besteht, ist keine.
			return;
		}

		const value = Number.parseInt(button.getAttribute('data-rs-bet') ?? '', 10);
		if (!Number.isInteger(value) || value < 1) {
			console.error('[reel-slot] Einsatztaste ohne gültiges data-rs-bet.', button);
			return;
		}

		for (const candidate of this.betButtons) {
			candidate.classList.toggle('rs-bet--selected', candidate === button);
		}
		...
		// Sofortige Auskunft statt einer Überraschung am Hebel: wer 10 wählt und
	...
	setLocked(flag) {
		this.locked = flag;
		this.betsGroup?.classList.toggle('rs-bets--locked', flag);
	}
	...
	/**
	 * Der Hebel wurde gezogen. Prüfen, gegebenenfalls absagen, sonst abbuchen.
	 *
	 ...
	 */
	onRound(event) {
		// Am Hebel gezogen — spätestens jetzt ist das Gerät bedient worden.
		this.engaged = true;
		...
		if (!this.machineCredit.canAfford(bet)) {
			...
			// startRound() liefert daraufhin false, der Hebel schnellt sauber
			// zurück (lever.js kennt diesen Fall ausdrücklich). Es wird NICHTS
		...
		this.pendingDebit = this.machineCredit.stake(bet).then(
			(result) => {
				if (result.ok !== true) {
					console.error(
						'[reel-slot] Der Einsatz konnte nicht abgebucht werden. Die Runde wird nicht ausgezahlt.',
						result
					);
					return false;
				}
			...
			(error) => {
				console.error('[reel-slot] Die Abbuchung ist fehlgeschlagen.', error);
				return false;
			}
		);
	}
	...
	async onResult(event) {
		try {
			...
			// Die Frage an Phase 8: will jemand diesen Gewinn übernehmen?
			const asked = this.emit('rs:payout', {
				win,
			...
			console.error('[reel-slot] Die Verrechnung der Runde ist fehlgeschlagen.', error);
		}
	}
	...
	onState(event) {
		...
		// Automat das, ohne dass man erst am Hebel ziehen muss.
		if (!this.locked) {
			this.reviewAffordability();
		}
	}
	...
	/**
	 * Ein Gewinn wurde gutgeschrieben – gleich, ob von Phase 7 oder später von
	 * der Risiko-Leiter.
	 *
	 ...
	 * ab Phase 8 auch für die Leiter, ohne dass dort etwas dafür getan wird.
	 *
	 ...
	 * Gehäuse.
	 *
	 * Gleiche Absicht wie data-rs-round (Phase 6), data-rs-risk-* (Phase 8) und
	 * data-rs-auto-* (Phase 9): der Zustand ist von außen ablesbar, und der
	 * Nachweis ist ein DOM-Lesevorgang statt einer Glaubensfrage.
	 *
	 * data-rs-mirror kommt aus machineCredit.mirror und NICHT aus dem
	 ...
	 * data-rs-bank, data-rs-total und data-rs-cashout gehören bank.js.
	 *
	 * @returns {void}
	 */
	syncAttributes() {
		const data = this.root.dataset;
		data.rsMachineCredit = String(this.machineCredit.amount);
		data.rsMirror = this.machineCredit.mirror;
	}

	/**
	 * Meldet alles ab. Wird beim Verlassen der Seite gerufen; Phase 9 verlangt
	 * „sauberes Aufräumen laufender Zeitgeber" ausdrücklich.
	 *
	 * @returns {void}
	 */
	destroy() {
		...
		// dorthin: reel-slot.js meldet teardown() für pagehide an, teardown()
		...
		// bereits dem Gerätekredit gutgeschrieben: reel-slot.js räumt die
		...
		this.betsGroup?.classList.remove('rs-bets--locked');

		// Der Zustand gehört zum laufenden Betrieb, nicht zum Gehäuse — genau
		// wie data-rs-round in machine.js.
		delete this.root.dataset.rsMachineCredit;
		delete this.root.dataset.rsMirror;
	}
}
