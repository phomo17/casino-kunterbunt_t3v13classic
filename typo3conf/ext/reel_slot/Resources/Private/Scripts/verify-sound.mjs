/**
 * Reel Slot – Nachweis des Klangausbaus
 * =========================================================
 *
 ...
 *
 *
 * WAS HIER BEWIESEN WIRD (CONCEPT.md B.10, Phase 4)
 * -------------------------------------------------
 *  1. Vor der ersten ECHTEN Nutzergeste entsteht kein Kontext und klingt
 ...
 *  3. Es gibt ZWEI hörbar verschiedene Absagen – und eine dritte, die stumm
 *     bleibt, weil das Ereignis die Blasenphase nie erreicht.
 *  4. CASH OUT klingt nur, wenn wirklich Geld geflossen ist.
 *  5. Die Leerlaufgeräusche: genau EIN Brummen je Gehäuse, es hängt wirklich
 *     am Ausgang, es schweigt vollständig, solange das Gerät arbeitet, und der
 *     Ton-Schalter wie der Registerkartenwechsel nehmen es mit.
 *  6. Zwei Gehäuse auf einer Seite stören einander nicht: das Abräumen des
 *     einen nimmt nur sein eigenes Brummen mit.
 *  7. Ein Dauerlauf über 25 Runden: NULL verworfene Stimmen, NULL wegen
 *     Mindestabstand verworfene Klänge, NULL Konsolenausgaben, und nach dem
 *     Abräumen kein zurückgebliebener Dauerklang.
 ...
 *   reel_slot         sound.js       das Klangpult dieses Geräts
 *                     counter.js     das Zählwerk, das rs:count sendet
 *
 ...
 *     ALS TEXT gelesen, genau diese Namen ersetzt und als data:-Modul geladen.
 *     Es ist Zeile für Zeile derselbe Code; nur der Modulname ist ein anderer.
 *     Dasselbe Verfahren wie in verify-machine-credit.mjs.
 *     counter.js hat keinen einzigen Import und wird deshalb unverändert
 *     geladen – genau wie paytable.js und risk-timing.js in den anderen
 *     Skripten.
 ...
 *     Instanz. Ohne das hätte jede Datei ihre eigene Buchführung und die
 *     Zahlen am Ende wären die von niemandem.
 *
 * Node hat weder Web Audio noch ein Dokument noch einen Browserspeicher.
 * Beides wird VOR dem Laden hier bereitgestellt (Abschnitte 1 bis 3).
 *
 *
 * WARUM DER AUSSCHLAG AUSGERECHNET UND NICHT GEMESSEN WIRD
 * -------------------------------------------------------
 * Gemessen wird der Ausschlag im Browser über einen AnalyserNode. Hier klingt
 * nichts, also gäbe eine Messung 0 – stats().peakLevel steht unter Node
 * deshalb immer auf 0,000, und das ist kein Fehler.
 *
 * Stattdessen wird gerechnet: für jeden Augenblick die Summe der Hüllkurven
 * aller gerade geplanten Stimmen, jede multipliziert mit allen Verstärkungen
 * auf ihrem Weg zum Ausgang (die Gesamtlautstärke eingeschlossen). Weil dabei
 * jede Auslöschung zwischen zwei Schwingungen ignoriert wird, ist das Ergebnis
 * eine OBERE SCHRANKE: der echte Ausschlag kann nie größer sein. Eine Messung
 * kann diese Schranke nicht unterlaufen.
 *
 * Zwei Vereinfachungen, beide zugunsten der Schranke:
 *   · Filter werden als durchlässig gerechnet. Ein Bandpass hat auf seiner
 *     Mittenfrequenz die Verstärkung 1; die benutzten Tief- und Hochpässe
 *     stehen mit Q ≤ 0,9 unterhalb jeder Resonanzüberhöhung.
 *   · Der Begrenzer (DynamicsCompressorNode) wird übergangen. Er kann den
 *     Pegel nur senken, nie heben.
 *
 * Gerechnet wird NICHT auf einem Raster, sondern auf den Stützstellen selbst:
 * jede Reglerbewegung und jeder Start und jedes Ende einer Quelle ist eine
 * Stützstelle. Zwischen zwei benachbarten Stützstellen ist jede einzelne
 * Hüllkurve monoton – deshalb genügt je Abschnitt der größere der beiden
 * Randwerte, und das ist beweisbar nicht zu wenig. Ein Raster könnte den
 * lautesten Augenblick verfehlen; diese Rechnung kann es nicht.
 *
 *
 * WARUM DIE ZEIT VIRTUELL IST
 * ---------------------------
 * 25 Runden dauern in Wirklichkeit gut zwei Minuten. setTimeout,
 ...
 * Zahlenfolge wie in verify-machine-credit.mjs. Damit liefert jeder Lauf
 * dasselbe Ergebnis und ein Fehlschlag ist nachstellbar. Erlaubt ist das,
 ...
 * Projekt ausnahmslos aus crypto.getRandomValues, und keine der fünf Dateien
 * zieht einen.
 *
 * Alle Vergleiche sind ganzzahlig, bis auf den Ausschlag – der wird mit <
 * geprüft. Im ganzen Skript steht kein Vergleich zweier Kommazahlen auf
 * Gleichheit.
 */
...
const REEL_JS = new URL('../../Public/JavaScript/', import.meta.url);

/** Mindestens 25 – CONCEPT.md B.10, Phase 4. */
const ROUNDS = 25;
 ...
 *
 * Eine schlichte Zuweisung genügt nicht: performance zum Beispiel ist in Node
 * kein gewöhnlicher Wert, sondern ein Zugriffspaar, und eine Zuweisung darauf
 * wirft in einem Modul (Module laufen immer im strengen Modus).
 *
 * @param {string} name
 * @param {*} value
 * @returns {void}
 ...
 * dasselbe Ergebnis liefert und ein Fehlschlag nachstellbar ist. Dieselbe
 * Formel wie in verify-machine-credit.mjs.
 *
 * @param {number} seed
 ...
 *
 * Die fälligen Zeitgeber werden ABGESCHRIEBEN, bevor sie laufen: ein Zeitgeber,
 * der sich selbst neu spannt (Klappern, Leerlauf), soll im selben Schritt nicht
 * ein zweites Mal drankommen.
 *
 * @param {number} ms
 * @returns {void}
 */
 ...
 *
 * Web Audio kennt vier Bewegungen. Drei davon werden hier genau abgebildet;
 * setTargetAtTime nähert sich seinem Ziel nur an und wird deshalb als der
 * GRÖSSERE der beiden Werte gehalten – die Annäherung kann ihn nie
 * überschreiten. Das ist für eine obere Schranke nie zu wenig und hält den
 * Regler zwischen zwei Stützstellen monoton, worauf die Rechnung beruht.
 */
class Param {
	/** @param {number} value */
	constructor(value) {
	...
	/**
	 * @param {{kind: string, t: number, v: number}} point
	 * @returns {Param}
	 */
	add(point) {
...
class StubAnalyser extends StubNode {
	...
	/**
	 * Hier klingt nichts, also ist der gemessene Ausschlag 0. Genau deshalb
	 * rechnet dieses Skript ihn selbst aus.
	 *
 ...
 *
 * Festgehalten und nicht später ermittelt, weil Knoten beim Abräumen getrennt
 * werden – ein später ermittelter Weg wäre dann keiner mehr.
 *
 * reaches sagt, ob der Weg wirklich am Ausgang ankommt. Das ist keine
 * Formsache: ein Klang, dessen Kette nirgends endet, wird gezählt und ist
...
class StubSource extends StubNode {
	...
	/**
	 * Die KÜRZERE der beiden Zeiten gewinnt: ein Abbruch kann einen Klang nur
	 * verkürzen, nie verlängern. stopAllVoices() ruft stop() auf Quellen, die
	 * ihr Ende schon kennen.
	 *
	 * @param {number} when
	 * @returns {void}
	 */
 ...
 *
 * Siehe Dateikopf. Gerechnet wird auf den Stützstellen, nicht auf einem
 * Raster; je Abschnitt zählt der größere der beiden Randwerte.
 *
...
/**
 * Alle Verstärkungen auf dem Weg einer Quelle zum Ausgang, miteinander
 * multipliziert. Die Quelle selbst hat höchstens die Auslenkung 1.
 *
...
/**
 * Ein Ereignis, dessen Ziel und dessen isTrusted diese Datei setzen darf.
 *
 * Node bringt seit Version 19 ein echtes CustomEvent mit – aber dessen target
 * ist nur lesbar und wird ausschließlich von einer echten Zustellung gesetzt,
 * und isTrusted lässt sich überhaupt nicht setzen. Beides wird hier gebraucht:
 * onRoundRefused() prüft das Ziel, onGesture() prüft isTrusted.
 */
class StubEvent {
...
class StubElement {
	 ...
	 * @param {string[]} [selectors] die Selektoren, unter denen dieses Element
	 *        gefunden werden soll. Ein echtes querySelector wäre für diese
	 *        Prüfung ein eigenes Programm; gebraucht werden genau drei.
	...
	/**
	 * Zustellung in drei Phasen: von oben herab (Erfassung), am Ziel, dann
	 * nach oben zurück (Blase). Genau diese Reihenfolge ist der Grund, warum
	 * das Klangpult rs:round am Dokument in der BLASENPHASE abhört – erst dort
	 * steht defaultPrevented fest.
	 *
	 * @param {StubEvent} event
	 * @returns {boolean}
	 */
...
/** Ein Browserspeicher im Arbeitsspeicher – wie in verify-machine-credit.mjs. */
const cells = new Map();
...
// sound.js meldet beim Laden einen Zuhörer für Speicherereignisse an. Der
// Abgleich zweier Registerkarten ist in verify-machine-credit.mjs bewiesen und
// wird hier nicht gebraucht; die Anmeldung darf trotzdem nicht werfen.
 ...
 *
 * @param {URL} url
 * @param {Array<[string, string]>} replacements Bare-Specifier → echte Adresse
 * @param {string} label für die Ausgabe
...
const boardUrl = await toModule(new URL('sound.js', REEL_JS),
	[[SOUND_NAME, SOUND_URL], [KIT_NAME, kitUrl], [IDLE_NAME, idleUrl]], 'reel_slot/sound.js');
const { MachineSound } = await import(boardUrl);
check(typeof MachineSound === 'function', 'reel_slot/sound.js geladen – das Klangpult');

const { NixieCounter } = await import(new URL('counter.js', REEL_JS).href);
check(typeof NixieCounter === 'function',
	'reel_slot/counter.js unverändert geladen – es hat keinen einzigen Import');
...
function buildMachine() {
	const root = document.append(new StubElement(document, ['.rs-machine']));
	const cabinet = root.append(new StubElement(document, ['.rs-cabinet']));
	const button = cabinet.append(new StubElement(document, ['[data-rs-sound]'], { rsSound: '' }));
	const element = root.append(new StubElement(document, ['.rs-nixie-group'], { rsDisplay: 'guthaben' }));
	const group = {
		element,
		length: 6,
		shown: [],
...
/* ==========================================================================
   6. DIE ACHT PRÜFUNGEN
   ========================================================================== */

console.log('\nPrüfung 1 — vor der ersten echten Geste bleibt alles still');
...
fire(one.root, 'rs:state', { from: 'idle', to: 'spinning' });
advance(500);
check(sound.stats().startedVoices === 0, 'ein Hebelzug ohne Geste erzeugt keine einzige Stimme');
...
one.root.dispatchEvent(new StubEvent('pointerdown', { bubbles: true, isTrusted: true }));
check(sound.contextState === 'running', 'der erste ECHTE Zeigerdruck legt den Kontext an');
advance(200);
check(humming() === 1, 'und genau ein Brummen setzt ein');
check(one.root.dataset.rsSoundSustained === '1', 'data-rs-sound-sustained meldet es am Gehäuse');
check(one.root.dataset.rsSoundSustainDropped === '0', 'data-rs-sound-sustain-dropped steht auf 0');

fire(one.root, 'rs:state', { from: 'spinning', to: 'idle' });
advance(300);

console.log('\nPrüfung 2 — der Zählklang hängt an der Fahrt, nicht am Betrag');

const counter = new NixieCounter(one.group);
let counted = { up: 0, down: 0 };
one.root.addEventListener('rs:count', (event) => {
...
fire(one.root, 'rs:count', { display: 'einsatz', value: 5, index: 1, steps: 4, direction: 'up' });
advance(200);
check(plays('count') === 12, 'eine andere Anzeige löst keinen Zählklang aus');

console.log('\nPrüfung 3 — zwei hörbare Absagen, eine stumme');

fire(one.root, 'rs:coin', { amount: 20, moved: 20, reason: 'ok', machineCredit: 120 });
advance(400);
check(plays('coin') === 1, 'ein angenommener Einwurf klingt nach Münze');

fire(one.root, 'rs:coin', { amount: 20, moved: 0, reason: 'nocash', machineCredit: 120 });
advance(400);
...
one.root.addEventListener('rs:round', refuse);
fire(one.root, 'rs:round', { bet: 1 }, { cancelable: true });
advance(500);
check(plays('round-refused') === 1, 'ABSAGE 2: ein abgelehnter Zug bekommt seinen eigenen tiefen Anschlag');
one.root.removeEventListener('rs:round', refuse);
...
document.addEventListener('rs:round', swallow, true);
fire(one.root, 'rs:round', { bet: 1 }, { cancelable: true });
advance(500);
check(plays('round-refused') === 1,
	'ABSAGE 3: in der Risiko-Leiter bleibt der Hebel stumm, obwohl der Zug abgelehnt ist');
document.removeEventListener('rs:round', swallow, true);

console.log('\nPrüfung 4 — CASH OUT klingt nur, wenn wirklich Geld fließt');

fire(one.root, 'rs:cashout', { moved: 0, capped: true, machineCredit: 250 });
advance(600);
check(plays('cashout-tray') === 0 && plays('cashout') === 0,
	'bei voller Kasse fließt nichts und es klingt nichts – das Schild sagt es');

fire(one.root, 'rs:cashout', { moved: 250, capped: false, machineCredit: 0 });
advance(800);
check(plays('cashout-tray') === 1, 'die Klappe der Auswurfschale');
check(plays('cashout') === 1, 'und danach die Münzen');

console.log('\nPrüfung 5 — die Leerlaufgeräusche');
...
fire(one.root, 'rs:state', { from: 'spinning', to: 'evaluating' });
fire(one.root, 'rs:risk', { phase: 'start', level: 1 });
fire(one.root, 'rs:state', { from: 'evaluating', to: 'result' });
advance(200);
check(idle.busy === true,
	'in der laufenden Risiko-Leiter bleibt es still, obwohl der Spielkern längst ruht');

fire(one.root, 'rs:risk', { phase: 'end' });
fire(one.root, 'rs:state', { from: 'result', to: 'idle' });
advance(200);
...
one.button.dispatchEvent(new StubEvent('click', { bubbles: true, isTrusted: true }));
...
console.log('\nPrüfung 6 — zwei Gehäuse auf einer Seite stören einander nicht');
...
two.root.addEventListener('rs:round', refuseTwo);
fire(two.root, 'rs:round', { bet: 1 }, { cancelable: true });
advance(500);
two.root.removeEventListener('rs:round', refuseTwo);
...
check(two.root.dataset.rsSoundSustained === '1',
	'das Attribut meldet die SEITE, nicht das Gehäuse – alle teilen einen Kontext');

console.log(`\nPrüfung 7 — Dauerlauf über ${ROUNDS} Runden`);

const BETS = [1, 2, 5, 10];
const FACTORS = [0, 0, 0, 0, 0, 0, 1, 1, 1, 3, 3, 5, 8, 10, 14, 20, 50];
...
for (let round = 1; round <= ROUNDS; round++) {
	...
	if (round % 5 === 1) {
		machineCredit += 100;
		fire(one.root, 'rs:coin', { amount: 100, moved: 100, reason: 'ok', machineCredit });
		counter.ramp(machineCredit);
		advance(600);
	}

	fire(one.root, 'rs:round', { bet }, { cancelable: true });
	machineCredit -= bet;
	counter.ramp(machineCredit);
	fire(one.root, 'rs:state', { from: 'idle', to: 'spinning' });
	advance(900);

	fire(one.root, 'rs:reelrest', { reel: 1 });
	advance(200);
	fire(one.root, 'rs:reelrest', { reel: 2 });
	advance(200);
	fire(one.root, 'rs:reelrest', { reel: 3 });
	advance(150);

	fire(one.root, 'rs:state', { from: 'stopping', to: 'evaluating' });
	advance(120);

	// Runde 13 ist FEST ein Jackpot. Der dichteste Augenblick des ganzen
	// Spiels – 1,53 s Jingle, unmittelbar danach Kasse, Münzkaskade und
	// Zählklang – darf nicht vom Zufall abhängen.
	const factor = round === 13 ? 100 : FACTORS[draw(FACTORS.length)];
	fire(one.root, 'rs:result', { factor, win: factor * bet, bet });
	advance(60);

	if (factor > 0) {
		const credited = factor * bet;
		machineCredit += credited;
		fire(one.root, 'rs:collect', { amount: credited, credited, capped: false, machineCredit });
		counter.ramp(machineCredit);
		advance(2600);
	}
...
check(one.root.dataset.rsSoundSustained === '0',
	'und das Gehäuse meldet das auch: data-rs-sound-sustained ist 0');
check(sustainedSources().length === 0, 'auch im Klanggraphen läuft keine Quelle ohne Ende mehr');

console.log('\nPrüfung 8 — der ausgerechnete Ausschlag');
...
console.log(failed
	? '\nERGEBNIS: der Klangausbau erfüllt mindestens eine Zusage aus B.10, Phase 4 nicht.'
	...
	+ 'beide Absagen klingen verschieden, und kein Dauerklang bleibt zurück.');
