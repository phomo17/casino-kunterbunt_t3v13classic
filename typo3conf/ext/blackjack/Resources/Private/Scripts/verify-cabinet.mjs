...
console.log('\nA-5  Kein fremder Hersteller-, Modell- oder Spieltitel');
{
	const NEGATIVLISTE = [
		...
		// Ergänzung dieser Extension (CONCEPT.md C.14, Kartenspiel-spezifisch):
		...
		// Kartenhersteller, Kartenmarken und benannte Rückenmuster — seit
		// Phase C5, weil dieses Gerät mit Karten spielt (Umsetzungsstück
		// C5d, Plan Abschnitt 4.32). Die beiden geläufigsten Rückenmuster
		// im US-amerikanischen Kartenspiel: das Rautenmuster der Marke Bee
		// und das Reiter-Rückenmuster der Marke Bicycle. Nach CONCEPT.md
		// V.7 Nr. 5 aus allgemein bekannten Marktfakten eingetragen (keine
		// Recherche aus dem Gedächtnis über Spezifika); die förmliche
		// Markenrecherche bei DPMA/EUIPO und die Bildrückwärtssuche des
		// fertigen Rückens bleiben offene, im Copyright-Bericht benannte
		// Aufträge an den Auftraggeber (Abschnitt 10, Punkte 2 und 3 des
		// Plans) — NICHT ABSCHLIESSEND GEKLÄRT.
