/**
 * Video Slot – Zählwerk für eine Nixie-Gruppe
 * ==========================================================
 *
 ...
 * .vs-nixie-group--overflow aus machine.css. Der Grund für den Überlauf: das
 ...
 * (nixie.js). Aus einer Fahrt über zwölf Schritte wird deshalb EINE Ansage
 * mit dem Endstand, ohne dass hier eine Zeile dafür stünde. snap() geht
 * denselben Weg und ist ebenfalls entprellt.
 *
 * Seit dem Audit-Nachlauf (2026-09-05/06, Befund N-04) reicht snap() das
 * zweite Argument announce durch: wallet.js ruft es beim allerersten
 * Anfangsstand mit false auf, damit die Röhren sofort ihren Startwert
 * zeigen, ohne dass ein Hilfsmittel ihn beim bloßen Laden der Seite
 * vorliest.
 *
 *
 ...
 * vs:count ausgelöst, das nach oben bis zum Gehäuse blubbert:
 *
 *   detail.display    'guthaben' | 'einsatz' | 'gewinn' — das Namensschild
 *                     der Gruppe aus data-vs-display
...
const OVERFLOW_CLASS = 'vs-nixie-group--overflow';
...
export class NixieCounter {
	 ...
	 *
	 * bubbles: true, damit die Meldung von der Röhrengruppe bis zum .vs-machine
	...
	announce(index) {
		...
		element.dispatchEvent(new CustomEvent('vs:count', {
			detail: Object.freeze({
				display: element.dataset.vsDisplay ?? '',
				value: this.value,
				index,
				steps: this.steps,
