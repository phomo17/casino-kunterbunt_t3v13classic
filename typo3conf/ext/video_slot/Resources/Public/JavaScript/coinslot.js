/**
 * Video Slot – der Geldeinwurf am Gehäuse
 * ======================================================
 *
 ...
 * --------------------
 * In der Geldreihe des Sockels, die Phase 5 dafür gezeichnet hat
 * (Cabinet.html, Abschnitt „Geldeinwurf am Sockel"; die Fassung steht im
 * Shell-SVG). Der gezeichnete Münzschlitz in der Mitte ist zugleich der
 * Absendeknopf des Formulars: man wirft ein, indem man in den Schlitz
 * greift. Zusätzlich löst die Eingabetaste im Feld denselben Weg aus, weil
 * das bei einem Formular so gehört.
 *
 * Die beiden unsichtbaren Beschriftungen (.vs-coinslot__legend) stehen schon
 ...
 * DIE MÜNZE
 * ---------
 * Die Einwurf-Animation ist reines CSS (@keyframes vs-coin-drop). Diese Datei
...
const SELECTOR_FORM = '[data-vs-coinslot]';

/** Ein Schnellwert-Knopf; sein Betrag steht im Attribut. */
const SELECTOR_ADD = '[data-vs-coin-add]';

/** Das Feld für den freien Betrag. */
const SELECTOR_INPUT = '[data-vs-coin-input]';

/** Der unsichtbare Knopf über dem gezeichneten Schlitz. */
const SELECTOR_SLOT = '[data-vs-coin-insert]';

/** Die fallende Münze. */
const SELECTOR_COIN = '[data-vs-coin]';

const COIN_CLASS = 'vs-coinslot__coin--drop';
const SLOT_CLASS = 'vs-coinslot__slot--flash';

/**
 * Laufzeit der Münzanimation in Millisekunden.
 *
 * Muss zu @keyframes vs-coin-drop in machine.css passen (dort 460 ms); die
...
export class CoinSlot {
	/**
	 * @param {HTMLElement} root das .vs-machine
	...
	wire() {
		for (const button of this.quickButtons) {
			this.listen(button, 'click', () => {
				const amount = Number.parseInt(button.dataset.vsCoinAdd ?? '', 10);
	...
	async insert(amount) {
		...
		this.root.dispatchEvent(new CustomEvent('vs:coin', {
