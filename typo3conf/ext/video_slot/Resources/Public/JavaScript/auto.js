 ...
 * result), nicht die Frage, wer die Taste bedient. Ein Zustand „auto" müsste
 ...
 * ---------------------------------------------------------------
 * Über das eingehende Ereignis vs:spin am Gehäuse. machine.js hat dafür genau
 ...
 * ABBRECHBARE vs:round wird gefeuert, die Kasse prüft und bucht ab, risk.js
 ...
 * nicht nachprüfbar — und in Chromium erteilt ein nachgemachtes Ereignis
 * ohnehin keine Nutzeraktivierung, der Klang bliebe also aus. vs:spin ist
 * die ausdrücklich dafür gebaute Naht (siehe Kopf von machine.js).
 *
 * vs:spin ist NICHT abbrechbar. Das Veto sitzt weiterhin an vs:round, wo es
 ...
 * vs:state ist bereits durch diesen Zuhörer gelaufen.
 *
 * Vier Gründe für diesen Weg:
 *
 *  · EINE WAHRHEIT. DECISIONS.md Phase 7 legt die Deckungsprüfung eindeutig in
 *    den vs:round-Zuhörer der Kasse. Eine zweite Prüfung hier wäre eine zweite
 ...
 * data-vs-auto-pending am Gehäuse und ist damit von außen messbar.
 *
 ...
 * vs:result neu gespannt wird, kann höchstens EINE angefangene Runde
 * stehenbleiben. Kein Stapel, kein Ereignisstau.
 *
 *
 * WAS „AB DEM NÄCHSTEN ZUG" GENAU HEISST
 * --------------------------------------
 * Gespannt wird beim vs:result der laufenden Runde. Der erste selbst
 * ausgelöste Zug beginnt also rund eine Sekunde nach der Auswertung.
 *
 * NICHT nach vs:risk phase:'end': dieses Ereignis kommt im Auto-Modus gar
 * nicht, weil das Angebot vorher verhindert wird. Und bei einer verlorenen
 * Runde käme überhaupt kein Risiko-Ereignis. vs:result ist der einzige Anker,
 * den JEDE Runde liefert.
 *
 *
 * KEINE RISIKO-LEITER — UND WAS DER SCHALTER WANN BEWIRKT
 * -------------------------------------------------------
 * Vor jedem Angebot fragt risk.js mit dem abbrechbaren vs:risk phase:'offer'.
 ...
 *   eingeschaltet, während RISK blinkt → vs:riskcollect nimmt den Gewinn
 ...
 * Deshalb wird vs:riskcollect beim Einschalten IMMER gesendet; im
 ...
 *    ohne jeden Tastendruck: tickFrame() befiehlt den Halt bei 1,2 / 1,8 /
 *    2,4 / 3,0 / 3,6 Sekunden ab Tastendruck (AUTO_STOP_S in machine.js).
 *    Der Auto-Modus fasst die Walzen nicht an und darf es nicht — sonst gäbe
 *    es zwei Bewegungsabläufe.
 *  · STOP, START, RISK und REWARD während des Auto-Modus: alle vier verhalten
 *    sich unverändert. STOP verkürzt die Runde, der nächste vs:result spannt
 ...
 * .length) ist nach 50 Zügen identisch, data-vs-auto-pending ist nie etwas
 ...
 * -------------------------------------
 * destroy() (aus pagehide, siehe video-slot.js) löscht den Zeitgeber, meldet alle
 * Zuhörer ab und macht die Taste dunkel. Es wird dabei KEIN vs:auto gemeldet:
 ...
 * ----------------------------------------------------------------------
 *   vs:spin          „Drück jetzt START." Nicht abbrechbar.
 *   vs:auto          detail: { on, reason, rounds }. reason ist 'user' oder
 *                     'insufficient'. Phase 10 hängt hier den Schaltklang an.
 *   vs:riskcollect   beim Einschalten; im Grundzustand wirkungslos.
 *
 * EREIGNISSE, DIE DIESE DATEI ABHÖRT
 * ----------------------------------
 *   vs:state         Rundenzustand mitführen.
 *   vs:result        die Pause spannen.
 *   vs:risk          bei phase:'offer' abbrechen.
 *
 ...
 *
 * CONCEPT.md 3.5 sagt „ca. 1 s Pause". Gemessen wird ab vs:result, also ab der
...
export class AutoPlay {
	...
	constructor(root) {
		...
		this.button = cabinet.querySelector('.vs-btn[data-vs-button="auto"]');
		 ...
		 * Spielkerns (machine.js, STATE.IDLE); bis zum ersten vs:state stimmt
		...
		this.listen(root, 'vs:result', () => this.onResult());
		this.listen(root, 'vs:risk', (event) => this.onRisk(event));

		// wirePressButton() aus press.js statt der eigenen wireButton() des
		// Reel Slot: die Taste geht damit auch mit der Tastatur (CONCEPT.md
		// B.1).
		this.disposeButton = wirePressButton(this.button, 'vs-btn--pressed', () => this.toggle());
		this.syncAttributes();
	}
	...
	enable() {
		...
		this.enabled = true;
		this.button?.classList.add('vs-btn--lit');
		this.syncAttributes();
		this.emit('vs:auto', { on: true, reason: 'user', rounds: this.rounds });
	...
	disable(reason) {
		...
		this.enabled = false;
		this.disarm();
		this.button?.classList.remove('vs-btn--lit');
		this.syncAttributes();
		this.emit('vs:auto', { on: false, reason, rounds: this.rounds });
	}

	/**
	 * Spannt die Pause — aber nur, wenn der Automat gerade frei ist.
	 *
	 * Läuft noch ein Zug, wird NICHT gespannt: dessen vs:result kommt ohnehin
	...
	fire() {
		...
		if (!FREE_STATES.includes(this.machineState)) {
			...
			// Abschaltung — der nächste vs:result spannt neu.
			this.syncAttributes();
			return;
		}

		// Derselbe Weg wie ein Tastendruck auf START. machine.js ruft daraufhin
		// startRound(), würfelt und feuert das abbrechbare vs:round.
		this.emit('vs:spin', {});

		// dispatchEvent() ist synchron: an dieser Stelle steht fest, ob die
		// Runde zustande kam. Kam sie nicht, hat jemand vs:round abgebrochen —
	 ...
	 * nicht." Wer vs:risk phase:'offer' abbricht, lässt vs:payout
	 ...
	 *
	 * Gleiche Absicht wie data-vs-round (Phase 6) und data-vs-risk-* (dieser Phase):
	 * der Zustand ist von außen ablesbar, und der Nachweis ist ein
	 * DOM-Lesevorgang statt einer Glaubensfrage. data-vs-auto-pending macht
	...
	syncAttributes() {
		...
		data.vsAuto = this.enabled ? 'on' : 'off';
		data.vsAutoRounds = String(this.rounds);
		data.vsAutoPending = this.timer === 0 ? '0' : '1';
	}
	 ...
	 *
	 * Kein vs:auto: eine Meldung in eine sterbende Seite ist Lärm, und
