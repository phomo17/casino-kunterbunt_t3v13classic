/**
 * Video Slot – das Bedienfeld der Risiko-Leiter
 * ============================================
 *
 * Wie am Reel Slot steht hier nur das AUSSEHEN. Das Spielwerk
 ...
 * kennen — er kennt weder .vs-btn noch vs:risk noch eine Nixie-Röhre.
 *
 *
 ...
 *     der Leiter (vs:payout, vs:round, vs:riskcollect hinein; vs:risk und
 *     vs:risktick hinaus).
 *
 ...
 *              START  → Gewinn wird gutgeschrieben, nächster Zug beginnt
 *              RISK   → Leiter startet
 ...
 *
 *   vs:payout   (von wallet.js, abbrechbar) → preventDefault(), Anspruch an die
 *                Leiter übergeben. Genau der Fall, den der Kopf von payout.js
 *                beschreibt.
 *   vs:round    (von machine.js, abbrechbar) → im Angebot durchlassen und dabei
 *                gutschreiben; in der Leiter abbrechen.
 *
 * Es gibt in dieser Datei KEINEN Import von Machine und keinen von Wallet.
 *
 *
 * WARUM vs:round IN DER ERFASSUNGSPHASE AM DOKUMENT ABGEHÖRT WIRD
 * -----------------------------------------------------------------
 ...
 *     offenem Gewinn START nicht drücken — die Kasse sagte „GUTHABEN ZU
 ...
 *
 * Die Alternative „in video-slot.js einfach vor der Kasse bauen" wäre eine
 * unsichtbare Kopplung an eine Zeilenreihenfolge. So steht die Zusage im Code.
 *
 *
 * DIE TASTEN WIRKEN BEIM DRÜCKEN — UND GEHEN MIT DER TASTATUR
 * ------------------------------------------------------------
 * wirePressButton() aus press.js löst bei pointerdown SOFORT aus: bei einem
 * Trefferfenster von 40 ms ist das keine Geschmacksfrage, click käme erst
 * beim Loslassen. Ein click mit detail === 0 ist der Tastaturweg und löst
 * ebenfalls aus. Der Reel Slot kann das an diesen vier Tasten heute nicht;
 * er wird in dieser Phase NICHT angefasst.
 *
 *
 * WARUM machine.js KEINEN ZUSTAND „risk" BEKOMMEN HAT
 * ---------------------------------------------------
 * Weil START in den beiden Abschnitten VERSCHIEDEN wirken muss: im Angebot
 ...
 * ist START in der Leiter durch das Veto oben. Ein Mechanismus, nicht zwei.
 *
 ...
 *
 *   Rundenbeginn bis vs:result   machine.js (clearResult / winDisplay.show)
 *   Angebot und Leiter            diese Datei
 *   ab dem nächsten vs:round     wieder machine.js
 *
 ...
 *
 * Beide Eigentümer legen je ein eigenes NixieGroup-Objekt auf DASSELBE
 * Element (wie am Reel Slot). Das ist unbedenklich, weil die
 * Eigentümerschaft zeitlich geteilt ist und beide denselben Ansagebereich
 * mit demselben Satzmuster füllen — nie gleichzeitig. Die STUFE-Gruppe
 * gehört dagegen ausschließlich dieser Datei.
 *
 *
 * WAS BEIM VERLASSEN DER SEITE PASSIERT
 * -------------------------------------
 * destroy() (aus pagehide, siehe video-slot.js) reicht an RiskLadder.destroy()
 ...
 *
 *
 * EREIGNISSE, DIE DIESE DATEI SENDET (alle am .vs-machine, alle bubbles)
 * -----------------------------------------------------------------------
 *   vs:risk phase:'offer'    ABBRECHBAR. Vor jedem Angebot. Wer abbricht,
 *                             verhindert die Leiter; die Kasse zahlt dann
 *                             sofort aus. Genau das verlangt CONCEPT.md 3.5
 *                             für den Auto-Modus.
 *   vs:risk phase:'start'    Leiter läuft an, Stufe 1.
 *   vs:risk phase:'hit'      Treffer. level und win sind bereits die neuen.
 *   vs:risk phase:'miss'     Fehlgriff. detail.lost ist der verlorene Betrag.
 *   vs:risk phase:'collect'  Ausgestiegen und gutgeschrieben.
 *   vs:risk phase:'end'      Grundzustand wiederhergestellt.
 *   vs:risktick              Das gemalte Lichtfeld hat gewechselt.
 *                             detail: { level, lit, on }. sound.js hängt hier
 *                             den Risiko-Blinkton an und filtert auf on.
 *
 * Diese sieben Ereignisse und ihre detail-Felder sind wortgleich zu denen des
 * Reel Slot (Teil A, Phase 6). auto.js und sound.js dieses Geräts docken
 * deshalb über denselben Vertrag an wie am Reel Slot, ohne dass die geteilte
 * Leiter selbst von einem Gerät weiß.
 *
 * EREIGNIS, DAS DIESE DATEI ENTGEGENNIMMT
 * ---------------------------------------
 *   vs:riskcollect           „Steig jetzt aus und schreib gut." Im
 ...
 * data-Attribute und die vs:-Ereignisse — und gibt jede Entscheidung an die
...
export class RiskPanel {
	...
	constructor(root) {
		...
		this.riskButton = cabinet.querySelector('.vs-btn[data-vs-button="risk"]');
		this.rewardButton = cabinet.querySelector('.vs-btn[data-vs-button="reward"]');

		/** Die beiden Risiko-Tasten, links und rechts neben RISK. */
		this.buttons = {
			left: cabinet.querySelector('.vs-btn[data-vs-button="risk-left"]'),
			right: cabinet.querySelector('.vs-btn[data-vs-button="risk-right"]'),
		};

		/** Die Lichtfelder darüber. */
		this.lamps = {
			left: cabinet.querySelector('.vs-lamp[data-vs-lamp="risk-left"]'),
			right: cabinet.querySelector('.vs-lamp[data-vs-lamp="risk-right"]'),
		};
		...
		this.listen(root, 'vs:payout', (event) => this.onPayout(event));
		this.listen(root, 'vs:riskcollect', () => this.ladder.collect());

		// Erfassungsphase am Dokument – siehe Dateikopf.
		this.listen(root.ownerDocument, 'vs:round', (event) => this.onRound(event), true);

		// Tastenweg für Zeiger UND Tastatur (press.js) statt der eigenen
		// wireButton() des Reel Slot. Siehe Dateikopf, Abschnitt „DIE TASTEN
		// WIRKEN BEIM DRÜCKEN — UND GEHEN MIT DER TASTATUR".
		this.disposers = [
			wirePressButton(this.riskButton, 'vs-btn--pressed', () => this.ladder.start()),
			wirePressButton(this.rewardButton, 'vs-btn--pressed', () => this.ladder.collect()),
			wirePressButton(this.buttons.left, 'vs-btn--pressed', () => this.ladder.guess('left')),
			wirePressButton(this.buttons.right, 'vs-btn--pressed', () => this.ladder.guess('right')),
		];
	 ...
	 * vs:payout abgebrochen. Lehnt die Leiter ab, bleibt vs:payout unangetastet
	 * und wallet.js zahlt wie immer sofort aus — ein Gewinn kann auf diesem Weg
	 * nicht verlorengehen.
	 *
	 * @param {CustomEvent} event abbrechbares vs:payout aus wallet.js
	 * @returns {void}
	 */
	onPayout(event) {
		...
		if (this.emit('vs:risk', { phase: 'offer', level: 0, win: amount }, true).defaultPrevented) {
			return;
		}
	...
	/**
	 * START wurde gedrückt – oder es wurde versucht.
	 *
	...
	onRound(event) {
		...
		if (this.ladder.phase === PHASE_LADDER) {
			// CONCEPT.md 3.4: „Hebel bzw. Starttaste sind in der Risiko-Leiter wirkungslos."
		...
		if (this.ladder.phase === PHASE_OFFER) {
			// CONCEPT.md 3.4: „START drücken → der Gewinn wird gutgeschrieben,
	 ...
	 *
	 * Die Namen und detail-Felder sind wortgleich die aus dem Reel Slot
	 * (Teil A, Phase 8) — auto.js und sound.js dieses Geräts docken deshalb
	 * über denselben Vertrag an.
	 *
	...
	report(message) {
		if (message.type === 'tick') {
			this.emit('vs:risktick', { level: message.level, lit: message.lit, on: message.on });
			return;
		}
	...
	render(view) {
		switch (view.reason) {
			...
			case 'settled':
				...
				// Der AUSGANG der Leiter, genau einmal angesagt: auf welcher
				// Stufe sie geendet hat. view.level trägt ihn noch — die
	 ...
	 */
	paintLit(view) {
		this.lamps.left?.classList.toggle('vs-lamp--on', view.lit === 'left');
		this.lamps.right?.classList.toggle('vs-lamp--on', view.lit === 'right');
		this.root.dataset.vsRiskLit = view.lit;
	}
	 ...
	 */
	paintLevel(view) {
		this.riskButton?.classList.remove('vs-btn--blink');
		this.buttons.left?.classList.add('vs-btn--lit');
		this.buttons.right?.classList.add('vs-btn--lit');
	 ...
	 */
	paintOffer(view) {
		this.riskButton?.classList.add('vs-btn--blink');
		this.rewardButton?.classList.add('vs-btn--lit');
		this.buttons.left?.classList.remove('vs-btn--lit');
		this.buttons.right?.classList.remove('vs-btn--lit');
		this.paintLit(view);
		// Der Satz des vorigen Ausgangs wird geräumt, sonst bliebe ein
		// gleichlautender Ausgang der nächsten Leiter stumm (announce()
		// vergleicht mit dem, was dasteht).
	 ...
	 */
	paintOff(view) {
		this.riskButton?.classList.remove('vs-btn--blink', 'vs-btn--lit');
		this.rewardButton?.classList.remove('vs-btn--lit');
		this.buttons.left?.classList.remove('vs-btn--lit');
		this.buttons.right?.classList.remove('vs-btn--lit');
		this.paintLit(view);
		this.showStep(0);
	 ...
	 *
	 * STILL: NixieGroup.html verlangt, dass für diese Gruppe der AUSGANG der
	 * Leiter angesagt wird und nicht jede einzelne Stufe. Eine Leiter über
	 * sechs Treffer erzeugte sonst sechs Ansagen in wenigen Sekunden.
	 * Angesagt wird stattdessen genau einmal, in render() bei 'settled'.
	 *
	...
	showStep(level) {
		...
		if (level <= 0) {
			this.stepGroup.clear(false);
		} else {
			this.stepGroup.show(level, false);
		}
		this.stepGroup.element.classList.toggle('vs-nixie-group--overflow', level > this.stepCeiling);
	}
	...
	showWin(amount) {
		...
		this.winGroup.element.classList.toggle('vs-nixie-group--overflow', amount > this.winCeiling);
	}

	/** Räumt ein Überlaufblinken der GEWINN-Gruppe ab. Siehe Dateikopf. */
	clearWinOverflow() {
		this.winGroup?.element.classList.remove('vs-nixie-group--overflow');
	}

	/**
	 * Schreibt den Zustand als data-Attribute ans Gehäuse.
	 *
	 * Gleiche Absicht wie data-vs-round in Phase 6: der Zustand ist von außen
	 * ablesbar, und der Nachweis ist ein DOM-Lesevorgang statt einer
	 * Glaubensfrage. data-vs-risk-side und -on machen zusätzlich die
	...
	syncAttributes(view) {
		...
		data.vsRiskPhase = view.phase;
		data.vsRiskLevel = String(view.level);
		data.vsRiskWin = String(view.win);
		data.vsRiskSide = String(view.sideMs);
		data.vsRiskOn = String(view.onMs);
		data.vsRiskLit = view.lit;
	}
