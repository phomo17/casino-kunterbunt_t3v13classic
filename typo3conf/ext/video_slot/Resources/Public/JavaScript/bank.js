/**
 * Video Slot – die Kasse am Gehäuse
 * ================================
 *
 * Zwei Bedienteile, ein Thema: der Gesamtbestand.
 *
 *   .vs-bank       das beleuchtete Fenster im Sockel, das die KASSE zeigt
 *   .vs-cashout    die Taste CASH OUT auf der Auswurfschale
 *
 ...
 *
 *   .vs-bank__value        der Kassenstand am Gerät
 *   .vs-bank__announce     der Ansage-Bereich für Hilfsmittel (Audit A-02)
 *   .vs-cashout            die Auszahl-Taste samt ihrem Leuchten und ihrer
 *                          Sperre (aria-disabled, Audit A-06)
 *   data-vs-bank           der Kassenstand, von außen ablesbar
 *   data-vs-total          Kasse + Gerätekredit. Die Bilanz aus CONCEPT.md
 *                          B.10, Phase 3 als EINE Zahl: sie darf sich durch
 *                          Einwurf, Einsatz, Gewinn und Auszahlung NIE ändern.
 *   data-vs-cashout        on | off
 *
 ...
 *
 * Sie schreibt auch NICHT data-vs-machine-credit und NICHT data-vs-mirror —
 ...
 *
 *   vs:cashout   { moved, capped, machineCredit }
 *
 ...
 * und das ist Absicht: video-slot.js räumt die Klangpulte als ERSTES ab (siehe
 ...
 * Betrag. Die Reihenfolge in video-slot.js ist entsprechend: Kassenanzeigen vor
 * den Verrechnungen.
 *
 *
...
export class Bank {
	...
	constructor(root, machineCredit, board) {
		...
		this.announceTemplate = this.announcer?.dataset.vsTextBank ?? '';
		if (this.announcer !== null && this.announceTemplate === '') {
			console.error('[video-slot] Am Kassenfenster fehlt data-vs-text-bank.');
			this.announcer = null;
		}
	...
	async cashOut() {
		try {
			...
			this.root.dispatchEvent(new CustomEvent('vs:cashout', {
			...
			console.error('[video-slot] Die Auszahlung ist fehlgeschlagen.', error);
		}
	}
	 ...
	 *
	 * data-vs-total ist der eigentliche Nachweis dieser Phase: die Summe aus
	 ...
	 * (Kassenfenster, Taste, data-vs-*) bekommt ihren Anfangsstand trotzdem
	...
	paint(reason) {
		...
		data.vsBank = String(bank);
		data.vsTotal = String(bank + machine);
		data.vsCashout = lit ? 'on' : 'off';
	}
	...
	destroy() {
		...
		// wie data-vs-round in machine.js und data-vs-auto-* in auto.js.
		delete this.root.dataset.vsBank;
		delete this.root.dataset.vsTotal;
		delete this.root.dataset.vsCashout;
	}
}
