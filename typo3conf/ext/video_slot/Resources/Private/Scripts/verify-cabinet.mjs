...
/**
 * Diese Datei selbst — nicht mehr für eine Namensausnahme (siehe A-13 unten:
 * die Negativliste steht seit Befund B-6 nicht mehr hier), sondern nur noch
 * dafür, dass A-13 die drei Zeichen ©/™/® ausschließlich in DIESER Datei
...
console.log('\nA-13 Kein fremder Hersteller-, Modell- oder Spieltitel');
{
	...
	const GEPRUEFT_A13 = alleDateien(EXT);
	const treffer = [];
	for (const datei of GEPRUEFT_A13) {
		...
		// und sichtbarem Text verschwindet damit. Weitere Ausnahme: in dieser
	...
	console.log('     Gegenprobe A-13-G: dieselbe Prüfung (musterFuer) muss einen gelisteten Namen auch in Versalien und ohne Trennzeichen erkennen');
	...
	// A-13 schlüge gegen die eigene Gegenprobe an. Der erfundene Text testet
	...
	check(gegenprobeGefunden.length >= 2,
		'A-13-G: sowohl der zusammengeschriebene Versalien-Name als auch der erste Listeneintrag werden erkannt',
		...gegenprobeGefunden);
}
