/**
 * Craps – Statistik für die Nachweisläufe
 * =========================================
 *
 * Entwicklerwerkzeug, kein Bestandteil der Website. Liefert den
 * Chi-Quadrat-Anpassungstest samt p-Wert (für die Gleichverteilung je
 * Würfel), den Chi-Quadrat-Unabhängigkeitstest über eine Kreuztabelle (für
 * die Unabhängigkeit der beiden Würfel — CONCEPT.md C.5.4 verlangt das für
 * Craps zusätzlich) und die empirische Streuung des Mittelwerts. Diese
 * Rechnungen prüfen verify-physics.mjs (Umsetzungsstück C6c) und
 * measure-dice.mjs (Messlauf M-C6-1) ihre jeweilige Zusage aus CONCEPT.md
 * C.5.4.
 *
 * WARUM DIESE DATEI NICHT ÜBERNOMMEN, SONDERN KOPIERT WIRD
 * ----------------------------------------------------------
 * Zwei Gründe. Erstens die Projektregel: keine Extension greift zur
 * Laufzeit auf eine Datei einer anderen zu — was übernommen wird, wird
 * KOPIERT, mit neu geschriebenem Kopfkommentar. Zweitens, und wichtiger:
 * eine Statistik, die selbst nicht geprüft ist, ist eine Annahme. Ein
 * übernommener p-Wert-Rechner wäre genau das. Deshalb hat diese Datei einen
 * eigenen Selbsttest, der unten beschrieben ist.
 *
 * gammaQ(), chiSquareUniform() und stdDevOfMean() sind zeichengleich zu
 * typo3conf/ext/blackjack/Resources/Private/Scripts/stats.mjs übernommen.
 * Neu ist ausschließlich chiSquareIndependence() — die Kartentisch-Fassung
 * kennt keine zwei gekoppelten Zufallsgrößen und braucht sie deshalb nicht.
 *
 ...
 * abgeschrieben — nicht aus gammaQ()/chiSquareUniform() hergeleitet. Für
 * chiSquareIndependence() kommen drei weitere, von Hand nachgerechnete
 * Stützstellen hinzu (siehe die Kommentare bei den einzelnen Prüfungen
 * unten) — ebenfalls unabhängig von jeder Zeile Code in dieser Datei.
 * Bricht der Selbsttest ab, brechen measure-dice.mjs und verify-physics.mjs
 * beim Start ebenfalls ab — ein Messlauf mit falscher Statistik soll gar
 * nicht erst laufen (CONCEPT.md C.5.4).
 *
 *   ddev exec node typo3conf/ext/craps/Resources/Private/Scripts/stats.mjs
 *
...
/**
 * Chi-Quadrat-Unabhängigkeitstest über eine Kreuztabelle.
 *
 * Die Frage, die CONCEPT.md C.5.4 für Craps zusätzlich stellt: hängt das
 * Ergebnis des zweiten Würfels davon ab, was der erste zeigt? Gezählt wird
 * dafür eine 6 × 6-Tabelle: Zeile = Auge des ersten Würfels, Spalte = Auge
 * des zweiten. Erwartet wird unter Unabhängigkeit in jeder Zelle
 * Zeilensumme · Spaltensumme / Gesamtzahl.
 *
 * Freiheitsgrade: (Zeilen − 1) · (Spalten − 1) = 25 bei zwei Würfeln. Das
 * ist der Unterschied zum Anpassungstest, wo es Zellen − 1 wären: die
 * Randsummen sind hier aus den Daten geschätzt und nicht vorgegeben.
 *
 * Zellen mit Erwartungswert 0 (eine ganze Zeile oder Spalte leer) werden
 * übersprungen und ihre Freiheitsgrade abgezogen — sonst käme eine Division
 * durch null heraus. Bei 500.000 Würfen tritt das nie ein; die Behandlung
 * steht hier für die Selbsttests mit kleinen Tabellen.
 *
 * @param {number[][]} tabelle
 * @returns {{chi2: number, df: number, p: number,
 *            maxCell: {row: number, col: number, observed: number, expected: number}}}
 */
export function chiSquareIndependence(tabelle) {
	const zeilenZahl = tabelle.length;
	const spaltenZahl = tabelle[0].length;

	const zeilenSumme = new Array(zeilenZahl).fill(0);
	const spaltenSumme = new Array(spaltenZahl).fill(0);
	let gesamt = 0;
	for (let r = 0; r < zeilenZahl; r++) {
		for (let c = 0; c < spaltenZahl; c++) {
			const wert = tabelle[r][c];
			zeilenSumme[r] += wert;
			spaltenSumme[c] += wert;
			gesamt += wert;
		}
	...
	let chi2 = 0;
	let maxCell = { row: -1, col: -1, observed: 0, expected: 0 };
	let maxAbweichung = -1;
	for (let r = 0; r < zeilenZahl; r++) {
		for (let c = 0; c < spaltenZahl; c++) {
			const erwartet = (zeilenSumme[r] * spaltenSumme[c]) / gesamt;
			if (erwartet <= 0) {
				continue;
			}
			const beobachtet = tabelle[r][c];
			const diff = beobachtet - erwartet;
			chi2 += (diff * diff) / erwartet;
			if (Math.abs(diff) > maxAbweichung) {
				maxAbweichung = Math.abs(diff);
				maxCell = { row: r, col: c, observed: beobachtet, expected: erwartet };
			}
		}
	}

	const leereZeilen = zeilenSumme.filter((s) => s === 0).length;
	const leereSpalten = spaltenSumme.filter((s) => s === 0).length;
	const df = (zeilenZahl - 1) * (spaltenZahl - 1)
		- leereZeilen * (spaltenZahl - 1)
		- leereSpalten * (zeilenZahl - 1);

	const p = df > 0 ? gammaQ(df / 2, chi2 / 2) : 1;

	return { chi2, df, p, maxCell };
}
 ...
 * geteilt durch die Wurzel des Stichprobenumfangs.
 *
 * @param {number[]} values
 ...
 * measure-dice.mjs und verify-physics.mjs (jeweils per Import) vor dem
...
export function selbsttest(melder) {
	...
	/*
	 * Drei zusätzliche, von Hand nachgerechnete Stützstellen für
	 * chiSquareIndependence() — Plan Teil 2, Abschnitt 4.32.
	 */

	// [[10,20],[20,40]] ist exakt das Produkt seiner Randverteilungen
	// (Zeilensummen 30/60, Spaltensummen 30/60, Gesamt 90 — jede Zelle
	// trifft Zeile·Spalte/Gesamt genau). Also chi2 = 0, df = 1, p = 1.
	const unabhaengig = chiSquareIndependence([[10, 20], [20, 40]]);
	pruefe(unabhaengig.chi2 === 0 && unabhaengig.df === 1 && unabhaengig.p === 1,
		'chiSquareIndependence bei [[10,20],[20,40]]: chi2 = 0, df = 1, p = 1 (Tabelle ist exakt das Produkt ihrer Randverteilungen)',
		`gefunden: chi2=${unabhaengig.chi2}, df=${unabhaengig.df}, p=${unabhaengig.p}`);

	// [[20,10],[10,20]]: Zeilensummen 30/30, Spaltensummen 30/30, Gesamt 60 —
	// alle vier Erwartungswerte sind 15. Jede Abweichung ist ±5, also
	// chi2 = 4 · 25/15 = 6,6667. df = 1. Der abgeschriebene kritische Wert
	// 6,635 gehört zu p = 0,010 (REFERENZTABELLE oben); 6,6667 liegt knapp
	// darüber, der zugehörige p-Wert muss also knapp UNTER 0,010 liegen.
	const gekoppelt = chiSquareIndependence([[20, 10], [10, 20]]);
	pruefe(Math.abs(gekoppelt.chi2 - 6.6667) < 0.001 && gekoppelt.df === 1,
		'chiSquareIndependence bei [[20,10],[10,20]]: chi2 ≈ 6,6667, df = 1 (von Hand nachgerechnet)',
		`gefunden: chi2=${gekoppelt.chi2}, df=${gekoppelt.df}`);
	pruefe(gekoppelt.p > 0.009 && gekoppelt.p < 0.010,
		'chiSquareIndependence bei [[20,10],[10,20]]: p liegt knapp unter der abgeschriebenen'
		+ ' Referenzstützstelle 0,010 bei chi2=6,635 (df=1) — 6,6667 liegt darüber, p also darunter',
		`gefunden: p=${gekoppelt.p.toFixed(6)}`);

	// [[5,5],[0,0]]: die zweite Zeile ist komplett leer. df sinkt auf 0
	// (Randfall), keine Division durch null.
	const randfall = chiSquareIndependence([[5, 5], [0, 0]]);
	pruefe(randfall.df === 0 && Number.isFinite(randfall.chi2),
		'chiSquareIndependence bei [[5,5],[0,0]]: df = 0 (eine Zeile ist leer), keine Division durch null',
		`gefunden: df=${randfall.df}, chi2=${randfall.chi2}`);
...
if (process.argv[1] && fileURLToPath(import.meta.url) === process.argv[1]) {
	console.log('\nCraps – Statistik für die Nachweisläufe: Selbsttest');
	console.log('======================================================\n');
	...
	console.log(bestanden
		...
		+ '\nchiSquareUniform(), chiSquareIndependence() und stdDevOfMean() stimmen'
		+ '\nan den Handbeispielen.'
