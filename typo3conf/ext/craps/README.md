...
rechnet die 47 Feldknöpfe des Tuchs samt ihrer Lage im Gitter aus
...
`Classes/BetLayout.php` ist die **einzige** Quelle der Lage jedes der 47
Felder im Gitter — `felt.css` zeichnet nur ein Raster aus `--cr-cell`-Spuren,
...
Die sechs Point-Zahlen (4, 5, 6, 8, 9, 10) stehen nebeneinander; Zahl Nummer
*j* (0 bis 5, in dieser Reihenfolge) belegt die Spalten **2j+1 bis 2j+3**.

| Zeile | Inhalt | Spalten |
|---|---|---|
| 1 | Puck-Spur (kein Feld) | — |
| 2 | Don't Come | je Zahl 2j+1…2j+3 |
| 3 | Come-Zahlen | je Zahl 2j+1…2j+3 |
| 4 | Odds hinter Come (links, come-odds) / Don't Come (rechts, dont-come-odds) | je Zahl geteilt: 2j+1…2j+2 / 2j+2…2j+3 |
| 5 | Place | je Zahl 2j+1…2j+3 |
| 6 | Field | 1…13 (volle Breite) |
| 7 | Come (links) / Don't Come (rechts) | 1…7 / 7…13 |
| 8 | Pass Line (links) / Don't Pass (rechts) | 1…7 / 7…13 |
| 9 | Odds hinter der Linie: Pass-Odds (links) / Don't-Pass-Odds (rechts) | 1…7 / 7…13 |
| 10 | Hardways (4, 6, 8, 10) | je Zahl `3k+1`…`3k+4`, k=0…3 |
| 11 | Einmalwetten (Any Seven, Any Craps, 2, 3, 11, 12) | je Feld `2k+1`…`2k+3`, k=0…5 |
...
## Die Wanne und ihre Maßordnung

Alle Zahlen stehen **einmal** in `Resources/Public/JavaScript/dice-geometry.js`
und werden von der Zeichnung (`Tray.html`) und von der Physik
(`dice-physics.js`, dort ausgeschrieben wiederholt, weil sie importfrei
bleiben muss) benutzt. `verify-tray.mjs` (D-1) und `verify-physics.mjs` (P-9)
gleichen beide Kopien bei jedem Lauf gegen diese eine Quelle ab.
...
**Sprunglink über 47 Tabstationen.** `.ck-skiplink.cr-felt__skip` (Partial
`Table/Craps/Felt`, direkt vor dem ersten Feldknopf) führt an den 47
Wettfeldern des Tuchs vorbei direkt zur Bedienleiste — ohne ihn müsste ein
Tastaturweg durch alle 47 Knöpfe, bevor er die Bedienung erreicht. Sichtbar
nur bei `:focus-visible`, damit die versteckte Position aus `base.css` nicht
durch eine bedingungslose Fassung aufgehoben wird.

**Rollbereich statt Schrumpfen (WCAG 2.2, SC 1.4.10).** Das Tuch ist
mindestens 33 rem breit und wird bei Platzmangel **gerollt**, nicht
verkleinert — die Zielgröße ist eine Zusage, die Bildschirmbreite keine.
Damit `overflow-x: auto` an `.cr-felt__scroll` überhaupt greift, muss
`min-inline-size: 0` die ganze Elternkette hindurch durchgelassen werden
(ein Flex-Kind hat sonst von Haus aus `min-width: auto` und schrumpft nie
unter seine Inhaltsbreite) — in dieser Extension von Anfang an richtig
gesetzt.

**Zielgröße 44 Bildpunkte auch für den schmalsten Odds-Streifen** (SC 2.5.8):
`--cr-cell` ist 2,75 rem; das schmalste Feld des Tuchs (ein Odds-Streifen) ist
genau eine Spur breit und erfüllt die Zielgröße damit ohne Ausnahme.
...
`verify-throw.mjs` (T-1 bis T-9), `verify-felt.mjs` (F-1 bis F-13),
...
`verify-tray.mjs` (Kennungen D-1 bis D-9): die Maßordnung steht in `Tray.html`
wörtlich so wie in `dice-geometry.js`, die vier Gummileisten liegen auf den
vier Bandeninnenkanten mit der richtigen `<pattern>`-Teilung, genau zwei
...
`verify-physics.mjs` (Kennungen P-1 bis P-10, Laufzeit unter zwei Sekunden):
...
Verwerfungsverfahren zieht sauber.
...
`verify-throw.mjs` (Kennungen T-1 bis T-9, rechnet mit `dice-physics.js`
...
und die Wanne bleibt dicht, auch bei sinnlosen Eingaben.

`verify-bets.mjs` (Kennungen B-1 bis B-12): `bets-craps.js` ist import- und
dokumentfrei, genau 47 Felder im richtigen Muster, jede Wettart aus Anhang H
...
Einzelhöchsteinsatz und der Summe aller Höchsteinsätze ohne Odds.
...
eine erschöpfende Probe über 47 Felder × 7 Zustände × 36 Paare, die
Bilanzprobe je Urteil, jede der sechs Absagen von `mayPlace()` sitzt an der
richtigen Stelle, und **der Quotennachweis am echten Zustandswerk** (W-15) —
eine von B-9 unabhängige zweite Herleitung derselben Zahlen.

`verify-felt.mjs` (Kennungen F-1 bis F-13): `BetLayout.php` und
...
`data-ck-field-label` mit demselben Inhalt, alle 47 Felder liegen innerhalb
des 12×11-Gitters ohne Überlappung, Zielgröße und Kontrast stimmen
...
je Wettart (13 Zeilen aus Anhang H, mit einer Toleranz aus vier
...
Version 0.3.0 (alpha). Beim Bauen wurde nicht hochgezählt (`CONCEPT.md` V.6);
die Zahl kam beim Feierabend des 2026-09-07 auf den projektweiten Stand, den
alle acht Extensions gemeinsam tragen. Phase C6 UND Phase C7 sind
