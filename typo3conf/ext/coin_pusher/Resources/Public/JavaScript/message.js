/**
 * Coin Pusher – das Meldungsschild
 * ==============================================
 *
 * Bauform aus video_slot/message.js, mit cp-Präfixen und dem Meldungssatz
 * dieses Geräts (locallang.xlf): insufficient, nocash, capped, invalid, norng.
 *
 * Die Tafel über dem Sichtfeld, nach Art der alten „TILT"-Schilder. Lauf 1
 * hat sie verborgen ins Markup gelegt und den Haken .cp-message--shown
 ...
 * auch bei einsprachigem Frontend. Das Schild bringt seine Texte deshalb
 * selbst mit, als data-Attribute aus der Sprachdatei:
 *
 *   data-cp-text-insufficient   GUTHABEN ZU GERING
 *   data-cp-text-nocash         KASSE ZU GERING
 *   data-cp-text-capped         KONTO VOLL
 *   data-cp-text-invalid        BETRAG UNGÜLTIG
 *   data-cp-text-norng          AUSSER BETRIEB
 *
 ...
 * ------------------------------------------------------
 * Machine/Cabinet.html gibt sie mit role="status" und LEER aus. Damit liest
 * ein Bildschirmleser jede Meldung von selbst vor — das ist einer der
 * Rückmeldekanäle dieses Geräts. Zwei Dinge hängen daran und dürfen nicht
...
const SHOWN_CLASS = 'cp-message--shown';
 ...
 *
 * Alle fünf liegen seit Lauf 1 im ausgelieferten Markup.
 */
const TEXT_KEYS = Object.freeze({
	insufficient: 'cpTextInsufficient',
	nocash: 'cpTextNocash',
	capped: 'cpTextCapped',
	invalid: 'cpTextInvalid',
	norng: 'cpTextNorng',
});
...
export class MessageBoard {
	/**
	 * @param {?HTMLElement} element das .cp-message, oder null
	 */
	constructor(element) {
	 ...
	 *
	 * @param {'insufficient'|'nocash'|'capped'|'invalid'|'norng'} key
	 * @returns {void}
	 */
	show(key) {
